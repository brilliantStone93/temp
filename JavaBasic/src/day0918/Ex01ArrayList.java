package day0918;
// ArrayUtil의 단점:
// 우리가 필요할때 마다 매번 비슷한 메소드들을 또 만들어주어야 한다.
...
public class Ex01ArrayList {
   public static void main(String[] args) {
      ...
      //    Q)"강사님... 기본형 데이터타입은 어레이리스트로 못써요? ㅠㅠ"
      //    A)넵 기본형 데이터타입은 사용하지 못합니다.
      //      그래서 자바가 기본형 데이터타입을 클래스처럼 만들어 놓은 포장 클래스(wrapper 클래스)가 존재합니다.
      ...
      // 어레이리스트는 java.util 패키지에 있으므로 우리가 import 해야한다.
      ...
      // 1. 어레이리스트의 현재크기를 알아볼때는 size()를 이용하면 된다.
      System.out.println("carList.size(): "+carList.size());
      
      // 2. 어레이리스트에 새로운 객체를 추가할때에는 add()를 이용하면 된다.
      carList.add(c1);
      System.out.println("carList.size(): "+carList.size());
      carList.add(c2);
      System.out.println("carList.size(): "+carList.size());
      ...
      // 3. 어레이 리스트에서 해당 위치에 있는 객체를 호출할때엔get() 메소드를 사용하면 된다.
      System.out.println("carList.get(1): " + carList.get(1));
      // get으로 호출한 객체는 배열의 볓번째 칸에 있는 객체다 처럼 우리가 그 객체의 메소드를 실행해 줄 수 있다.
      System.out.println("carList.get(0).getType(): " + carList.get(0).getType());
      
      // 4. 어레이리스트에서 객체를 제거할 때에는 2가지 방법이 가능하다.
      //	A) 인덱스로 삭제하기
      //	      인덱스로 삭제할 때에는 remover(index)를 하면 된다.
      //	   해당 인덱스가 삭제되면 그 이후에 있던 인덱스가 하나씩 앞으로 땡겨진다.
      ...
      // remove에 객체를 집어넣으면
      // ArrayList는 해당 어레이리스트이 처음부터 끝까지
      ...
      System.out.println("carList.inedexOf(c2) : "+carList.indexOf(c2));
      //  	만약 해당 리스트에 그 객체가 존재하지 않으면 -1이 린턴된다.
      System.out.println("carList.indexOf(c4) : "+ carList.indexOf(c4));
      
      // 6. carList에서 해당 객체가 존재하는지 존재하지 않는지를 체크할 때에는 contains() 메소드를 실행하면 된다.
      System.out.println("carList.contain(c2) : "+ carList.contains(c2));
      System.out.println("carList.contain(c4): "+carList.contains(c4));
      
      //따라서, ArrayList를 제대로 사용하고 싶다면,
      // 우리가 <>에 넣어줄 클래스에 
      // equals()를 정확하게 만들어주는 것이 중요하다!! 이게 핵심!
